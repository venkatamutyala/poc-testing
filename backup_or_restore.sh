...
aws s3 cp s3://${S3_BUCKET_NAME}/${S3_OBJECT_KEY_FOR_PLUGIN_ZIP} all-in-one-wp-migration-unlimited-extension.zip
...
if [ -n "$BACKUP_TO_RESTORE" ]; then
    ...
    aws s3 cp s3://${S3_BUCKET_NAME}/backups/${BACKUP_TO_RESTORE} /var/www/html/wp-content/ai1wm-backups/restore.wpress
    ...
    aws s3 cp /var/www/html/wp-content/ai1wm-backups/${LATEST_BACKUP} s3://${S3_BUCKET_NAME}/backups/${LATEST_BACKUP}
